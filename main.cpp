...
GLdouble angulo = 0.0;
GLdouble lx = 0.0f, lz = -1.0f,ly = 1.0f;
GLdouble x = 0.0f, z = 5.0f, y = 1.0f;
...
void cameracontrole(int key, int xx, int yy)
{
	float amount = 0.5f;
	float amount2 = 0.1f;
	switch (key)
...
{
	glutPostRedisplay();
	glutTimerFunc(velocidade, timerFunc, 0);
}
void configCamera()
{
...
int main(int argc, char **argv)
{
	...
	glutInitWindowSize(largura_, altura_);
	glutCreateWindow("Sistema Solar");
	glutDisplayFunc(gerarplanetas);
	glutTimerFunc(10, timerFunc, 0);
	glutSpecialFunc(cameracontrole);
	configCamera();
	configTextura();
	configIluminacao();
	carregarTexturas();
